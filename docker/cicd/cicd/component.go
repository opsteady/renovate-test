...
func (d *DockerCicd) Initialize(defaultComponent component.DefaultComponent) {
	...
	buildArgs := map[string]string{
		"FROM_IMAGE":               fmt.Sprintf(
			...
			"v2.1.0" // renovate: datasource=docker registryUrl=opsteadyos.azurecr.io depName=opsteadyos.azurecr.io/sig-storage/csi-node-driver-registrar versioning=semver
		),
